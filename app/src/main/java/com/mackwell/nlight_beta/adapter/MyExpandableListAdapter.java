...
import com.mackwell.nlight_beta.models.Loop;
...
//            use instead of checkedList.get(j).size();
//            int a = checkedList.get(j).size();
    	      int a = listDataChild.get(listDataHeader.get(j)).size()+1;
    			for(int i=0; i<a;i++){
